                        ...
                        <div class="card-header">
                          <div class="row">
                            ...
                            <div class="col-sm-6 text-right">
                                 <a href="<?=base_url()?>buyer/order-history" class="btn btn-sm btn-primary">Back</a>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="card-body card-body--padding--2">
                            <div class="row">
                                <?php foreach($list->seller_list as $rows)
                                {
                                    ...
                                    if($list->seller_id==$bank->buyer_id){?>
                                <div class="col-md-4">
                        ...
                        <div class="card-body card-body--padding--2">
                            <div class="row">
                                <div class="col-md-4 col-lg-4 col-sm-12">
                                    ...
                                    <p>20 % Payment Confirmation: 
                                        <a href="<?=main_url?>/<?=$list->first_invoice?>" download><?php echo str_replace('/uploads/first_invoice/', '',$list->first_invoice);?>
                                        </a> &nbsp;
                                        <b><i class="far fa-pencil text-success" data-modal-trigger="first-invoice<?=$list->bidding_id?>" type="button">
                                            </i> 
                                            <?php if($list->first_invoice){?>&nbsp;
                                            <i class="far fa-trash ml-2 text-danger" data-modal-trigger="delete-first<?=$list->bidding_id?>" type="button"></i>
                                        </b><?php }?>
                                    </p> 
                                    <?php } if($list->fully_utr){?>

                                    <p>80 % UTR Number: <?=$list->fully_utr?></p>
                                    <p>80 % Payment Confirmation: 
                                        <a href="<?=main_url?>/<?=$list->fully_invoice?>" download><?php echo str_replace('/uploads/final_invoice/', '', $list->fully_invoice)?>
                                        </a> &nbsp;
                                        <b><i class="far fa-pencil text-success" data-modal-trigger="fully-invoice<?=$list->bidding_id?>" type="button">
                                            </i> 
                                            <?php if($list->fully_invoice){?>&nbsp;
                                            <i class="far fa-trash ml-2 text-danger" data-modal-trigger="delete-final<?=$list->bidding_id?>" type="button">
                                            </i>
                                        </b><?php }?>
                                    </p> 
                                    <?php } if($list->buyer_purchase_order){?>
                                   <p>Purchase Order: 
                                        <a href="<?=main_url?>/<?=$list->buyer_purchase_order?>" download><?php echo str_replace('/uploads/purchase_order_buyer/', '', $list->buyer_purchase_order)?>
                                        </a> &nbsp;
                                        <b><i class="far fa-pencil text-success" data-modal-trigger="purchase-order<?=$list->bidding_id?>" type="button">
                                        </i> 
                                        <?php if($list->buyer_purchase_order){?>&nbsp;
                                        <i class="far fa-trash ml-2 text-danger" data-modal-trigger="delete-purchase<?=$list->bidding_id?>" type="button">
                                        </i>
                                        </b><?php }?>
                                    </p> 
                                ...
                                <div class="col-md-<?=$row?> col-lg-<?=$row?> col-sm-12">
                                  <h5>Downloads</h5>
                                   <h6>Order Confirmation Invoice: 
                                    <a href="<?=$list->proforma_invoice?>" target="_blank" download><?=str_replace(main_url, '',$list->proforma_invoice)?>
                                    </a>
                                  </h6> 
                                   ...
                                   <h6>Proforma Invoice: <a href="<?=main_url?>/<?=$list->seller_performa_invoice?>" target="_blank" download><?=str_replace('/uploads/performa_invoice_seller/', '',$list->seller_performa_invoice)?></a>
                                   </h6>
                               <?php } if($list->challan || $list->laurry || $list->main_invoice){?>
                                   <h6>MTC: <?=$list->mtc?></h6>
                                   <h6>Packaging List: <?=$list->packing_list?></h6>
                                   <h6>Delivery Challan: <a href="<?=main_url?>/<?=$list->challan?>" target="_blank" download><?=str_replace('/uploads/challans/', '',$list->challan)?></a>
                                   </h6>
                                   <h6>Lorry Receipt: 
                                    <a href="<?=main_url?>/<?=$list->laurry?>" target="_blank" download><?=str_replace('/uploads/laurry/', '',$list->laurry)?></a>
                                    </h6> 
                                   <h6>Final Tax Invoice By Seller: <a href="<?=main_url?>/<?=$list->main_invoice?>" target="_blank" download><?=str_replace('/uploads/invoice_seller/', '',$list->main_invoice)?></a>
                                   </h6> 
                                    <h5>Photos By Seller:</h5> 

                                    <?php foreach($list->biddimage_list as $ims){?>
                                        <h6><a href="<?=main_url?>/<?=$ims->order_images?>" target="_blank" download><?=str_replace('/uploads/order_images/', '',$ims->order_images)?></a>
                                        </h6> 
                                    <?php } }?>
